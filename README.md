...
## 1. Quick Start (new clone)

新しく clone した直後は、まず外部リポジトリを取得してから設定反映とビルドに進みます。

<details>
<summary><code>uv</code> / <code>vcstool</code> のインストール手順 (mac / Linux)</summary>

`uv tool install ...` で入れた `vcstool` は、このプロジェクト配下ではなくユーザー環境に入ります。
...
# go2.repos に定義した外部リポジトリを取得
uvx --from vcstool vcs import --force < go2.repos
(uvxはuv tool runのエイリアス)
...
`uv` がまだ無い環境では、先に `uv` を導入してください。

コンテナ内では `docker/Dockerfile.slam` で `python3-vcstool` を入れていますが、これはイメージ内依存のためであり、
この README の初回セットアップ手順とは分けて考えます。

必要なら、Jetson / Ubuntu ホストで OS パッケージの `vcstool` を使うこともできますが、
README 上の標準手順は `uv` に寄せます。
...
コンテナ作成後、`src/ros` 配下の ROS パッケージと、`src/zenoh` / `src/zenoh-plugin-ros2dds` の Rust ワークスペースを順にビルドします。
`zenoh` 系は `colcon build` とは別に Rust ワークスペースとしてビルドします。
Dockerfile に `cargo` / `rustc` を含めているため、`make build TARGET=<target>` 後は追加セットアップなしでそのまま実行できます。
Dockerfile を更新した場合は、先に `make build TARGET=<target>` でイメージを再作成してください。
...
- `cmd_vel_control` の依存である `unitree_go` / `unitree_api` は `src/ros/unitree_ros2` 配下から解決されるため、事前に `uvx vcs import --force < go2.repos` で取得しておきます。
- ビルド対象の定義は `Makefile` 上部の定数に寄せているため、将来パスを増減する場合も `colcon-build` / `zenoh-build` 本体の分岐は増やさない方針です。
